...
def crear_instancia(request):
    if request.method == 'POST':
        try:
            ...
            contraseña_convenio = request.POST.get('contraseña_convenio')
            ...
            # Crear convenio asociado al usuario
            ...
            certificados_seleccionados = [
                'certificado_persona_natural', 'certificado_persona_juridica', 'certificado_pertenencia_empresa',
                'certificado_Comunidada_Académica', 'certificado_Función_Publica_SIIF_Nación', 'certificado_Profesional_Titulado',
                'certificado_Persona_Natural_Con_RUT', 'certificado_Facturacion_Electronica_Persona_JurÍdica',
                'certificado_Facturacion_Electronica_Persona_Natural', 'certificado_Funcion_Publica', 'certificado_Representante_Legal'
            ]
            
            tipos_certificados = []
            for certificado in certificados_seleccionados:
                if request.POST.get(certificado):
                    tipos_certificados.append(certificado)
            print('certificados:', tipos_certificados)

            
            vigencias = request.POST.getlist('vigencias')
            formatos = request.POST.getlist('formatos')
            print('vigencias:',vigencias)
            print('formatos:', formatos)
            
            if tipos_certificados and vigencias and formatos:
                # Convertir vigencias a enteros
                try:
                    vigencias = [int(v) for v in vigencias]
                    print('hola:', vigencias)
                except ValueError:
                    return JsonResponse({"error": "Formato incorrecto para vigencias"}, status=400)
                print('certificados:', tipos_certificados)    
                for tipo in tipos_certificados:
                    ...
                    print('nuevo certificado:', nuevo_certificado)
                
                return JsonResponse({"message": "Instancia y certificados guardados correctamente"}, status=200)
            else:
                return JsonResponse({"error": "Falta información de certificados, vigencias o formatos"}, status=400)
